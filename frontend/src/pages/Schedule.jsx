...
function monthLabel(monthPart) {
    return MONTH_OPTIONS.find((m) => m.value === monthPart)?.label ?? monthPart;
}
...
function Schedule() {
    ...
    async function loadData(activeMonth) {
        ...
        try {
            const scheduleEntries = await getCalendar(activeMonth);
    ...
    async function handleDelete() {
        try {
            await deleteCalendar(editingId);
            cancelEditor();
            await loadData(month);
        } catch (requestError) {
            setError(requestError.message);
            ...
            <Box className="hero-card" sx={{ mb: 2 }}>
                ...
                <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
                    ...
                    <Typography variant="subtitle1" sx={{ minWidth: 160, textAlign: "center" }}>
                        {monthLabel(selectedMonthPart)} {selectedYear}
                    </Typography>
                    <IconButton onClick={() => shiftMonth(1)} size="small">
            ...
            <Dialog open={isModalOpen} onClose={cancelEditor} fullWidth maxWidth="sm">
                ...
                <Box component="form" onSubmit={handleSubmit}>
                    ...
                    <DialogActions>
                        ...
                        <Box sx={{ flex: 1 }} />
                        <Button onClick={cancelEditor} disabled={saving}>
                            Cancel
                        </Button>
